...
class PHASAgent:
    ...
    def __init__(
        ...
        # Shared-anchor weight (collaboration<->competition axis). Default 1.0
        # reproduces the published model exactly (shared target at full weight);
        # shared_weight=0 removes the common goal, leaving only the opposing
        # private targets (pure competition).
        ...
        # Desired beliefs (goal alignment applied). The raw target densities are
        # kept so a dynamic gamma update can recompute the blend (Extension 3).
        self._b_star_raw = b_star
        self.b_star = (
            shared_weight * b_star[0] + (1 - alignment) * b_star[1],
            shared_weight * b_star[0] + (1 - alignment) * b_star[2],
        )
    ...
    def step(self):
        ...
        # Dynamic gamma update (Extension 3). The updated gamma must propagate
        # into the desire distributions and the partner-action prior: before
        # this fix the update rewrote self.alignment (and the trace) only, so
        # b_star, q_star and p_ap kept their construction-time values and the
        # gamma dynamics were recorded but behaviourally inert (verified by
        # bit-identical trajectories against the static-gamma configuration
        # under per-condition reseeding).
        if self.dynamic_gamma and self._prev_fe is not None:
            delta_fe = self._prev_fe - current_fe  # positive = improvement
            new_alignment = np.clip(
            ...
            if new_alignment != self.alignment:
                ...
                self.b_star = (
                    self.shared_weight * self._b_star_raw[0] + (1 - self.alignment) * self._b_star_raw[1],
                    self.shared_weight * self._b_star_raw[0] + (1 - self.alignment) * self._b_star_raw[2],
                )
                self.q_star = (
                    variational_density(self.b_star[0]),
                    variational_density(self.b_star[1]),

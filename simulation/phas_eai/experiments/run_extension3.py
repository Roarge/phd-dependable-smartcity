...
Experimental design:
  - Factorial baselines A3 (static alignment=0.5) and A4 (static ToM+alignment),
    the paper's own levels, distinct from the published Model 3 and 4.
...
FACTORIAL_BASELINES = {
    ...
    "A3": dict(
        model_label="A3: Goal Alignment (static, paper levels)",
        perceptiveness=MAX_SENSE_PROBABILITY,
        alterity=(0, 0), alignment=0.5,
    ),
    "A4": dict(
        model_label="A4: ToM + Alignment (static, paper levels)",
        perceptiveness=MAX_SENSE_PROBABILITY,
...
def run_extension3(no_of_cycles=3, seed=None, output_dir=None, epochs=EPOCHS):
    ...
    for key, params in {**FACTORIAL_BASELINES, **EXT3_CONDITIONS}.items():
    ...
    gamma_baselines = {k: results[k] for k in FACTORIAL_BASELINES if k in results}

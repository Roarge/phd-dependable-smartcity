...
Experimental design:
  ...
  - Compare system free energy and convergence against the K1 baseline
    and the A4 factorial level (the paper's own ToM + alignment levels).
...
def run_extension2(no_of_cycles=3, seed=None, output_dir=None, epochs=EPOCHS):
    ...
    for key, params in {**FACTORIAL_BASELINES, **EXT2_CONDITIONS}.items():

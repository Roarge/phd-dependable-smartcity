...
# ---------------------------------------------------------------------------
# Factorial baselines (K1 matches the published Model 1; A2 to A4 are
# this project's own levels on the same 2x2 structure)
# ---------------------------------------------------------------------------
...
FACTORIAL_CONDITIONS = {
    ...
    "A2": dict(
        model_label="A2: Theory of Mind (paper levels)",
        perceptiveness=MAX_SENSE_PROBABILITY,
        alterity=(0.5, 0.5), alignment=0,
    ),
    "A3": dict(
        model_label="A3: Goal Alignment (paper levels)",
        perceptiveness=MAX_SENSE_PROBABILITY,
...
def run_extension1(no_of_cycles=3, seed=None, output_dir=None, epochs=EPOCHS):
    ...
    for key, params in FACTORIAL_CONDITIONS.items():
    ...
    kaufmann_results = {k: results[k] for k in FACTORIAL_CONDITIONS}
    plot_aggregate_comparison(
        kaufmann_results,

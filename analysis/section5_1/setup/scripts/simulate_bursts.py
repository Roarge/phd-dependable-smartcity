...
def save_manifest(result_dir: Path) -> None:
    """Write the section manifest, mirroring the schema of sections 5.2 to 5.7.

    Reads the freshly written proof_status.json so the theorem records stay
    single-sourced, and records the determinism block (the seed set in this
    script, the Agg backend, and the PYTHONHASHSEED exported by
    analysis/run_section5_1.sh).
    """
    proof_status = json.loads((result_dir / "proof_status.json").read_text())
    manifest = {
        "section": "5.1",
        "theorem_status": [
            {k: t[k] for k in ("name", "status", "file", "line_range")}
            for t in proof_status["theorems"]
        ],
        "assumptions": proof_status["assumptions"],
        "file_paths": {
            "figure_bursts_pdf": "analysis/section5_1/result/fig_5_1_complexity_bursts.pdf",
            "figure_bursts_png": "analysis/section5_1/result/fig_5_1_complexity_bursts.png",
            "figure_growth_pdf": "analysis/section5_1/result/fig_multiplicative_option_growth.pdf",
            "figure_joint_pdf": "analysis/section5_1/result/fig_joint_intervention_effect.pdf",
            "table_metrics_csv": "analysis/section5_1/result/burst_metrics.csv",
            "table_composition_csv": "analysis/section5_1/result/tab_5_1_multiplicative_composition.csv",
            "proof_file": "analysis/section5_1/setup/lean/ComplexityBursts.lean",
        },
        "determinism": {
            "seed": SEED,
            "backend": "Agg",
